...
test_data_file = os.path.join(data_path, 'deep-b3-test-data.pkl')
model_file = os.path.join(model_path, 'deep-b3.pth')
...
if not os.path.exists(test_data_file):
    logging.error('test data file not exist, please re-train it')
    exit(0)
...
tfile = open(test_data_file, 'rb')
test_data = pickle.load(tfile)
tfile.close()

# label [0, 1], 0--BBB+, 1--BBB-
label = test_data.valid_ds.y.items

deep_b3 = load_learner(model_path, 'deep-b3.pth')

deep_b3.data = test_data

preds, y = deep_b3.get_preds()
preds = preds.numpy()[:,0]
fpr, tpr, thresholds = metrics.roc_curve(label, preds, pos_label=0)
auc = metrics.auc(fpr, tpr)
cutoff = find_best_cutoff(fpr, tpr, thresholds)
logging.info('the auc on the test data is {0}'.format(auc))
#logging.info('find the best cut-off for the model, {0}'.format(cutoff))
y_pred_label = list(map(lambda x: 1 if x <cutoff else 0, preds))
conf_matrix = metrics.confusion_matrix(label, y_pred_label)
...
logging.info('the acc on the test data is {0}'.format(acc))
logging.info('the sn on the test data is {0}'.format(sn))
logging.info('the sp on the test data is {0}'.format(sp))

test_result = pd.DataFrame()
test_result['id'] = test_id
test_result['smi'] = test_smi
test_result['label'] = label
test_result['pred_label'] = y_pred_label
test_result['prob'] = preds
